...
import { PaginatedResponse, ClubDTO, CreateClub } from 'boxeo-shared';

export const getClubs = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const [clubsResult, countResult] = await Promise.all([
      pool.query(clubsQuery, queryParams),
    ...
    const clubs: ClubDTO[] = clubsResult.rows.map(row => ({
      id: row.id,
      nombre: row.nombre,
    ...
    const response: PaginatedResponse<ClubDTO> = {
      data: clubs,
      total,
      page: parseInt(page),
...
export const getClubById = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
export const createClub = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { nombre, ubicacion, descripcion }: CreateClub = req.body;
    ...
    if (req.user.role === 'administrador_general' && req.body.admin_user_id) {
      ...
      const adminUser = await pool.query(
        ...
        [req.body.admin_user_id, 'administrador_club']
      );
    ...
    const clubResult = await pool.query(
      `INSERT INTO clubs (nombre, ubicacion, descripcion, admin_user_id)
       VALUES ($1, $2, $3, $4)
       RETURNING id, nombre, ubicacion, descripcion, admin_user_id, activo, created_at, updated_at`,
      [nombre, ubicacion, descripcion, adminUserId]
    );
...
export const updateClub = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const existingClub = await pool.query(
      'SELECT id, admin_user_id FROM clubs WHERE id = $1',
      [id]
    );
...
export const deleteClub = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    await pool.query(
      'UPDATE clubs SET activo = false, updated_at = CURRENT_TIMESTAMP WHERE id = $1',
      [id]
    );
...
export const getClubBoxers = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    ...
    const [boxersResult, countResult] = await Promise.all([
      pool.query(boxersQuery, [id, limit, offset]),
