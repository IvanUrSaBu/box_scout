...
import { PaginatedResponse, BoxerDTO, CreateBoxer } from 'boxeo-shared';

export const getBoxers = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { page = 1, limit = 10, search = '', sort = 'created_at', order = 'desc', club_id } = req.query as any;
    ...
    const [boxersResult, countResult] = await Promise.all([
      pool.query(boxersQuery, queryParams),
    ...
    // Transformar datos
    const boxers: BoxerDTO[] = boxersResult.rows.map(row => ({
      id: row.id,
      user_id: row.user_id,
      club_id: row.club_id,
      nombre: row.nombre,
    ...
    const response: PaginatedResponse<BoxerDTO> = {
      data: boxers,
      total,
      page: parseInt(page),
...
export const getBoxerById = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
export const createBoxer = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { nombre, peso_categoria, fecha_nacimiento, club_id }: CreateBoxer = req.body;
    ...
    if ((req.user.role === 'administrador_general' || req.user.role === 'administrador_club') && req.body.user_id) {
      ...
      const userCheck = await pool.query(
        'SELECT id, role FROM users WHERE id = $1 AND role = $2 AND activo = true',
        [req.body.user_id, 'boxeador']
      );
    ...
    const existingBoxer = await pool.query(
      'SELECT id FROM boxers WHERE user_id = $1',
      [boxerUserId]
    );
    ...
    if (club_id) {
      const clubCheck = await pool.query(
        'SELECT id, admin_user_id FROM clubs WHERE id = $1 AND activo = true',
        [club_id]
      );
    ...
    const boxerResult = await pool.query(
      `INSERT INTO boxers (user_id, club_id, nombre, peso_categoria, fecha_nacimiento)
       VALUES ($1, $2, $3, $4, $5)
      ...
      [boxerUserId, club_id || null, nombre, peso_categoria, fecha_nacimiento]
    );
...
export const updateBoxer = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    ...
    const existingBoxer = await pool.query(
      'SELECT id, user_id, club_id FROM boxers WHERE id = $1',
      [id]
    );
...
export const deleteBoxer = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    ...
    await pool.query(
      'UPDATE boxers SET activo = false, updated_at = CURRENT_TIMESTAMP WHERE id = $1',
      [id]
    );
...
export const getBoxerFights = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    ...
    // Verificar que el boxeador existe
    const boxerExists = await pool.query(
      'SELECT id FROM boxers WHERE id = $1',
      [id]
    );
