...
import { PaginatedResponse, FightDTO, CreateFight } from 'boxeo-shared';

export const getFights = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { 
      page = 1, 
      limit = 10, 
    ...
    const [fightsResult, countResult] = await Promise.all([
      pool.query(fightsQuery, queryParams),
    ...
    const filteredFights = fightsResult.rows.filter(row => {
      ...
      if (!row.torneo_id || row.torneo_publico) return true;
      
      // Si es administrador general, mostrar todos
      if (req.user?.role === 'administrador_general') return true;
    ...
    // Transformar datos
    const fights: FightDTO[] = filteredFights.map(row => ({
      id: row.id,
    ...
    const response: PaginatedResponse<FightDTO> = {
      data: fights,
...
export const getFightById = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;

    const fightResult = await pool.query(
      `SELECT 
        f.id, f.torneo_id, f.boxeador1_id, f.boxeador2_id, f.peso_categoria,
        f.resultado, f.fecha_combate, f.ubicacion, f.notas, f.created_at, f.updated_at,
        b1.nombre as boxeador1_nombre, b1.victorias as boxeador1_victorias, b1.derrotas as boxeador1_derrotas,
...
export const createFight = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { 
      torneo_id, 
      boxeador1_id, 
    ...
    const boxersCheck = await pool.query(
      'SELECT id, nombre, activo FROM boxers WHERE id IN ($1, $2)',
      [boxeador1_id, boxeador2_id]
    );
    ...
    if (torneo_id) {
      const tournamentCheck = await pool.query(
        'SELECT id, estado, organizador_club_id FROM tournaments WHERE id = $1',
        [torneo_id]
      );
    ...
    const fightResult = await pool.query(
      `INSERT INTO fights (torneo_id, boxeador1_id, boxeador2_id, peso_categoria, fecha_combate, ubicacion, notas)
      ...
      [torneo_id || null, boxeador1_id, boxeador2_id, peso_categoria, fecha_combate, ubicacion, notas]
    );
...
export const updateFight = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    ...
    const existingFight = await pool.query(
      `SELECT f.id, f.torneo_id, t.organizador_club_id, t.estado 
       FROM fights f 
       LEFT JOIN tournaments t ON f.torneo_id = t.id 
       WHERE f.id = $1`,
      [id]
    ...
    if (req.user?.role === 'administrador_club' && fight.torneo_id) {
      const clubResult = await pool.query(
        'SELECT id FROM clubs WHERE admin_user_id = $1 AND id = $2',
        [req.user.id, fight.organizador_club_id]
      );
...
export const deleteFight = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    ...
    await pool.query('DELETE FROM fights WHERE id = $1', [id]);
