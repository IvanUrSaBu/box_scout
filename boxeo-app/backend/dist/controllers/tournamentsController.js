...
exports.getTournamentFights = exports.deleteTournament = exports.updateTournament = exports.createTournament = exports.getTournamentById = exports.getTournaments = void 0;
const database_1 = require("../config/database");
const getTournaments = async (req, res, next) => {
    try {
        const { page = 1, limit = 10, search = '', sort = 'created_at', order = 'desc', estado } = req.query;
        ...
        if (req.user?.role !== 'administrador_general') {
            if (req.user?.role === 'administrador_club') {
                ...
                const clubResult = await database_1.pool.query('SELECT id FROM clubs WHERE admin_user_id = $1', [req.user.id]);
    ...
    `;
        ...
        const [tournamentsResult, countResult] = await Promise.all([
            database_1.pool.query(tournamentsQuery, queryParams),
...
const getTournamentById = async (req, res, next) => {
    try {
        const { id } = req.params;
        const tournamentResult = await database_1.pool.query(`SELECT 
        t.id, t.nombre, t.descripcion, t.organizador_club_id, t.es_publico,
        t.fecha_inicio, t.fecha_fin, t.estado, t.ubicacion, t.created_at, t.updated_at,
        c.nombre as organizador_club_nombre, c.ubicacion as organizador_club_ubicacion,
        COUNT(f.id) as combates_count
      ...
      LEFT JOIN fights f ON t.id = f.torneo_id
      WHERE t.id = $1
      GROUP BY t.id, c.nombre, c.ubicacion`, [id]);
...
const createTournament = async (req, res, next) => {
    try {
        const { nombre, descripcion, es_publico, fecha_inicio, fecha_fin, ubicacion } = req.body;
        if (!req.user) {
        ...
        if (req.user.role === 'administrador_club') {
            const clubResult = await database_1.pool.query('SELECT id FROM clubs WHERE admin_user_id = $1 AND activo = true', [req.user.id]);
            if (clubResult.rows.length === 0) {
        ...
        if (req.user.role === 'administrador_general' && req.body.organizador_club_id) {
            const clubCheck = await database_1.pool.query('SELECT id FROM clubs WHERE id = $1 AND activo = true', [req.body.organizador_club_id]);
        ...
        const tournamentResult = await database_1.pool.query(`INSERT INTO tournaments (nombre, descripcion, organizador_club_id, es_publico, fecha_inicio, fecha_fin, ubicacion)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id, nombre, descripcion, organizador_club_id, es_publico, fecha_inicio, fecha_fin, 
                 estado, ubicacion, created_at, updated_at`, [nombre, descripcion, organizadorClubId, es_publico, fecha_inicio, fecha_fin, ubicacion]);
...
const updateTournament = async (req, res, next) => {
    try {
        ...
        const existingTournament = await database_1.pool.query('SELECT id, organizador_club_id FROM tournaments WHERE id = $1', [id]);
...
const deleteTournament = async (req, res, next) => {
    try {
        ...
        await database_1.pool.query('UPDATE tournaments SET estado = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', ['cancelado', id]);
        res.json({
            success: true,
...
const getTournamentFights = async (req, res, next) => {
    try {
        const { id } = req.params;
        const { page = 1, limit = 10 } = req.query;
        const offset = (page - 1) * limit;
        // Verificar que el torneo existe y permisos
        const tournamentResult = await database_1.pool.query('SELECT id, es_publico, organizador_club_id FROM tournaments WHERE id = $1', [id]);
        if (tournamentResult.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Torneo no encontrado',
...
exports.getTournamentFights = getTournamentFights;
